# -*- coding: utf-8 -*-
"""
Created on Mon Nov 29 21:26:48 2021
...
#        # Hasta aqui
        self._support_output_directory = output_directory
        self._max_total_time_support = max_total_time
        self._max_eval_time_support = max_eval_time
        self._support_verbosity = verbosity
        self._support_store = store
...
#        self._max_eval_time_support = max_eval_time
        ...
        if os.path.isfile(path):  
            ...
            if not self._support_output_directory:
                output_directory = f"gama_{str(uuid.uuid4())}"
            self.output_directory = os.path.abspath(os.path.expanduser(output_directory))
            if not os.path.exists(self.output_directory):
                os.mkdir(self.output_directory)
    
            register_stream_log(self._support_verbosity)
            if self._support_store in ["logs", "all"]:
                log_file = os.path.join(self.output_directory, "gama.log")
                log_handler = logging.FileHandler(log_file)
                log_handler.setLevel(logging.DEBUG)
                log_format = logging.Formatter("[%(asctime)s - %(name)s] %(message)s")
